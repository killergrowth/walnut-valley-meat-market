...
function porkForm() {
  ...
  const grind = radio('pork-grind','pgrind-ground','ground','Ground Pork',true) +
    ...
    `<div id="pork-grind-also-wrap" style="display:none"><div class="border-t border-stone-200 mt-3 pt-3"><p class="text-xs text-stone-500 mb-2 italic">Whole Hog only — also add:</p><div class="flex items-center gap-2 mb-2"><input type="checkbox" id="pgrind-also-sausage" class="custom-radio flex-shrink-0"/><label for="pgrind-also-sausage" class="text-sm text-stone-800 cursor-pointer">Also add Breakfast Sausage</label></div><div id="pork-also-sausage-sub" class="ml-6 mt-1 mb-1 grid grid-cols-2 gap-3" style="display:none"><div><label class="text-xs text-stone-600">Spice Level</label>${sel('pork-also-sausage-level',['Mild','Medium','Hot'],'Mild')}</div><div><label class="text-xs text-stone-600">Package Size</label>${sel('pork-also-sausage-pkg',['1 lb packages','2 lb packages'],'1 lb packages')}</div></div></div></div>` +
    `<div id="pork-grind-also-ground-wrap" style="display:none"><div class="border-t border-stone-200 mt-3 pt-3"><p class="text-xs text-stone-500 mb-2 italic">Whole Hog only — also add:</p><div class="flex items-center gap-2 mb-2"><input type="checkbox" id="pgrind-also-ground" class="custom-radio flex-shrink-0"/><label for="pgrind-also-ground" class="text-sm text-stone-800 cursor-pointer">Also add Ground Pork</label></div><div id="pork-also-ground-sub" class="ml-6 mt-1 mb-1" style="display:none"><label class="text-xs text-stone-600">Package Size</label>${sel('pork-also-ground-pkg',['1 lb packages','2 lb packages'],'1 lb packages')}</div></div></div>`;
