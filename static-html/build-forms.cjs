...
function beefForm() {
  ...
  const round = radio('beef-round','rnd-steak','steak','Round Steak',true) +
  ...
  const chuck = `<div class="space-y-4"><div><p class="font-medium text-stone-800 mb-2">Chuck Roast</p>${radio('beef-chuck','chk-roast','roast','Roast',true)}${radio('beef-chuck','chk-grind','grind','Grind',false)}</div><div><p class="font-medium text-stone-800 mb-2">Arm Roast</p>${radio('beef-arm','arm-roast','roast','Roast',true)}${radio('beef-arm','arm-grind','grind','Grind',false)}</div></div>`;
  ...
  const grind = `<div class="grid md:grid-cols-2 gap-4"><div><label class="block text-sm font-bold text-stone-700 mb-1">Leanness</label>${sel('beef-leanness',['80% Lean','85% Lean','90% Lean'],'85% Lean')}</div><div><label class="block text-sm font-bold text-stone-700 mb-1">Package Size</label>${sel('beef-pkg',['1 lb packages','2 lb packages'],'1 lb packages')}</div></div>`;
  ...
  return fsec(2,'Select Quantity','','',qty,true) +
    ...
    fsec(6,'Sirloin Section','[{"term":"Sirloin Steaks","description":"Lean, boneless steaks. Great for grilling or fajitas."},{"term":"Grind","description":"Added to ground beef."}]','Lean and flavorful.',sirloin,false) +
    ...
    fsec(8,'Chuck Section','[{"term":"Chuck Roast","description":"Classic pot roast cut. Incredible when braised."},{"term":"Arm Roast","description":"Slightly leaner round-bone roast. Great for pot roast."}]','Great for pot roast.',chuck,false) +
    fsec(9,'Brisket','[{"term":"Keep Whole","description":"Full brisket intact - 12-16 lbs. Perfect for smoking."},{"term":"Cut in Half","description":"Cut lengthwise, fat left on."},{"term":"Grind","description":"Added to ground beef."}]','',brisket,false) +
...
function porkForm() {
  ...
  const grind = radio('pork-grind','pgrind-ground','ground','Ground Pork (1 lb or 2 lb packages)',true) +
  ...
  return fsec(2,'Select Quantity','','',qty,true) +
    ...
    fsec(8,'Ham','[{"term":"Cure (Traditional Ham)","description":"Salt-cured and smoked - just like a traditional holiday ham."},{"term":"Do Not Cure (Fresh Pork Roast)","description":"Kept fresh - a very large pork roast."},{"term":"Put Into Grind","description":"Ham meat added to ground pork."}]','',ham,false) +
